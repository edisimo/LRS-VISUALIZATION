...
const topics=['mapping','planning','trajectory','avoidance'];
...
await page.selectOption('#demo','Inflation');await page.locator('#param-radius').fill('1.2');await page.locator('#param-margin').fill('0.8');await page.waitForTimeout(250);if(!(await page.locator('#metrics').textContent()).includes('20'))throw Error('Inflation layer count incorrect');await page.locator('#reset').click();await page.waitForTimeout(200);if(await page.locator('#param-radius').inputValue()!=='0.25')throw Error('Reset failed');await page.locator('#top').click();await page.locator('#play').click();await page.waitForTimeout(500);await page.locator('#play').click();if((await page.evaluate(()=>window.__lab.progress))<=0)throw Error('Playback failed');
